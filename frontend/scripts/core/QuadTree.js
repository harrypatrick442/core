var QuadTree = new (function() {
    //var mapQuadrantToDistance={};
	var nLevels = 19;
	this[S.GET_QUADRANTS_FOR_LAT_LNG] = function(latLng)
	{
		...
		while (level < nLevels)
		{
			...
			quads.push({[S.LEVEL]:level,[S.VALUE]:lastQuadN});
			level++;
		}
		return quads;
	};
	this[S.GET_QUADS_FOR_RADIUS] = function(latLng, radiusKm)
		...
		getAllChildQuadrants = function(lastQuadN, latFrom, latTo, lngFrom, lngTo, level, minLevelPickFrom, maxLevel)
		{
			...
			for (var i = 0; i < 4; i++)
			{
				...
				if (toPush)
				{
					...
					children.push({[S.LEVEL]: level, [S.VALUE]: newQuadN});
				} else {
